...
cities_path = paste(getwd(), "/lists/cities/", sep = "")
fluency_city <- list.files(path =cities_path, pattern = "*.xlsx", full.names = TRUE) %>% 
  lapply(readxl::read_excel) %>% 
  bind_rows %>%
  select(subject, domain,capital)%>%
  rename(word = capital)

final_fluency = rbind(fluency, fluency_city)
...
set.seed(100)
test = sample(unique(final_fluency %>% pull(subject)) , 120)
