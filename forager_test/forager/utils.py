...
def prepareData(path, domain):
    ...
    df = pd.read_csv(path, header=None, engine='python', sep=None, encoding='utf-8-sig', names=['SID', 'entry', 'rt'])

    # load labels
    labels = pd.read_csv('data/lexical_data/' + domain + '/frequencies.csv', names=['word', 'logct', 'ct']) 
